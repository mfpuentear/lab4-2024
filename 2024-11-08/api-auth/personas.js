...
router.post(
  ...
  async (req, res) => {
    ...
    if (req.user.rol !== "admin") {
      res.status(401).send({
        mensaje: "El usuario no tiene permisos para crear una persona",
      });
      return;
    }
...
router.put(
  ...
  async (req, res) => {
    ...
    if (req.user.rol !== "admin") {
      res.status(401).send({
        mensaje: "El usuario no tiene permisos para modificar una persona",
      });
      return;
    }
...
router.delete(
  ...
  async (req, res) => {
    ...
    if (req.user.rol !== "admin") {
      res.status(401).send({
        mensaje: "El usuario no tiene permisos para quitar una persona",
      });
      return;
    }
